...
import TSSettingsRruleDateList from "./settingsrruledatelist";
...
class TSSettingsRruleAdd extends React.Component {

    constructor(props) {
        super(props);
        this.template = {
            name:"",
            rulemode:props.rulemode||"ruleadd",
            frequency:3,
            start:new Date(),
        ...
        this.elements=[
            //            {title:"rulemode",          attr:"rulemode",        type:"text", tooltip: ""},
            ...
            {title:"start date",    attr:"start",       type:"datetime-local", tooltip: "The recurrence start. Besides being the base for the recurrence, missing parameters in the final recurrence instances will also be extracted from this date. If not given, <code>new Date</code> will be used instead." },
            {title:"until date",    attr:"until",       type:"datetime-local", tooltip: "If given, this must be a <code>Date</code> instance, that will specify the limit of the recurrence. If a recurrence instance happens to be the same as the <code>Date</code> instance given in the <code>until</code> argument, this will be the last occurrence."},
    ...
    setData(rrule) {
        const obj={
            ...
            rulemode:               rrule.rulemode ?        rrule.rulemode : "ruleadd",
...
*/
    ...
    validateByweekday(attr) {
        var [name,value] = attr.split("_"); // eslint-disable-line
        if (isNaN(value)) return "invalid value";
        value = parseInt(value);
        ...
        return (
            <>
                <Grid container spacing={3}>
                    <Grid item xs={6}>
                        <h3>{(this.state.rulemode=="ruleadd") ? I18n.t("Adding Timerule") : I18n.t("Deleting Timerule")}</h3>
                        {this.elements.map((el) => this.renderFormfield(el))}
                    </Grid>
                    <Grid item xs={6}>
                        <TSSettingsRruleDateList
                            rrule={this.getData}
...
export default withStyles(styles)(TSSettingsRruleAdd);