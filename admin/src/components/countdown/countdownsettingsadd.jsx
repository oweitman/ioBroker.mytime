...
class CountdownSettingsAdd extends React.Component {
    ...
    handleInput(e) {
        const errmsg = {errmsg:{}};
        if (e.target.name == "name") e.target.value = e.target.value.replace(/[^0-9a-zöÖüÜäÄß\.]/gi, ""); //eslint-disable-line
    ...
    setData(counter){
        const cd = util.calcCountdownFromMiliSeconds(counter.timer||0);
        return {
            name:       counter.name||"",
            days:       cd.days||"",
            hours:      cd.hours||"",
...
CountdownSettingsAdd.propTypes = {
