...
class mytimeCountdown {
        ...
        for (const cdIndex in countdownConfig) {
            ...
            for (const keyprop in this.stateCounterTemplate) {
                if (!datapoints[[cdConfig.name, keyprop].join('.')]) {
                    ...
                    if (keyprop == 'cmd' || keyprop == 'start' || keyprop == 'end') {
    ...
    doObserverServerCountdown() {
        ...
        this.ioUtil.setAdapterTimeout('doObserverServerCountdown', this.doObserverServerCountdown.bind(this), 1000);
    }
    /**
    ...
    doCmd(idParts, state) {
        ...
        data.configParsed = JSON.parse(data.config);
        //eslint-disable-next-line
        const regex = new RegExp("([=+\\-\\#\\$]{1})(!?)(.*)|(start|pause|stop|end|reset|setstop2timer|setstop2zero|setstop2rerun|save)", "");
            ...
            if (command) {
                ...
                if (command == 'end') {
                    switch (data.action) {
                        ...
                        case 'stop': {
                            // v4.1
                            let start = now - data.end > data.timer ? now : data.end;
                            if (data.configParsed.stopbehaviour == 'timer') {
                                this.countdownInfo[name].start = 0;
                                this.countdownInfo[name].end = 0;
                                this.ioUtil.setState('start', 0, this.countdownPath, name);
                                this.ioUtil.setState('end', 0, this.countdownPath, name);
                            ...
                            if (data.configParsed.stopbehaviour == 'zero') {
                                this.countdownInfo[name].start = 0;
                                this.countdownInfo[name].end = 0;
                                this.ioUtil.setState('start', 0, this.countdownPath, name);
                                this.ioUtil.setState('end', 0, this.countdownPath, name);
                            ...
                            if (data.configParsed.stopbehaviour == 'rerun') {
                                this.countdownInfo[name].start = start;
                                this.countdownInfo[name].end = start + data.timer;
                                this.ioUtil.setState('start', start, this.countdownPath, name);
                                this.ioUtil.setState('end', start + data.timer, this.countdownPath, name);
                                action = 'run';
                            }
                ...
                if (command == 'setstop2rerun') {
                    data.config.stopbehaviour = 'rerun';
                    this.countdownInfo[name].config.stopbehaviour = 'rerun';
                    this.ioUtil.setState('config', JSON.stringify(data.config), this.countdownPath, name);
                }
                if (command == 'save') {
    ...
    convertInfo2Config(newConfiguration) {
        ...
        if (behavior.stopbehaviour == undefined || behavior.stopbehaviour == '') {
            behavior.stopbehaviour = 'timer';
        }
        return {
    ...
    processMessages(msg) {
        ...
        if (msg.command === 'getServerTime') {
            this.ioUtil.logdebug('send getServerTime');
            this.getServerTimeMsg(msg);
        }
        if (msg.command === 'setCountdownConfig') {
            this.ioUtil.logdebug('send setCountdownConfig');
            this.setCountdownConfigMsg(msg);
        }
    }

    /**
     * Handles the 'setCountdownConfig' message command by setting the specified countdown config.
     *
     * This method is called when the adapter receives a 'setCountdownConfig' message.
     * It logs a debug message indicating that it is processing a 'setCountdownConfig' command.
     * It retrieves the message data and calculates the new timer value based on the provided seconds,
     * minutes, hours, and days. It then updates the countdown info with the new timer value and
     * config, and sends a response back to the sender using the provided callback function if it exists.
     *
     * @param msg - The message object containing the command 'setCountdownConfig' and the callback function.
     */
    setCountdownConfigMsg(msg) {
        this.ioUtil.logdebug('setCountdownConfigMsg ');
        let data = msg.message;
        let value =
            Number(data.seconds) * 1000 +
            Number(data.minutes) * (1000 * 60) +
            Number(data.hours) * (1000 * 60 * 60) +
            Number(data.days) * (1000 * 60 * 60 * 24);

        this.countdownInfo[data.name].config = JSON.stringify({ stopbehaviour: data.behavior });
        this.countdownInfo[data.name].timer = value;
        this.ioUtil.setStateAsync('timer', value, this.countdownPath, data.name);
        this.ioUtil.setStateAsync('config', this.countdownInfo[data.name].config, this.countdownPath, data.name);
    ...
    /**
     * Handles the 'getServerTime' message command by sending the current server time.
     *
     * This method logs a debug message indicating that it is processing a 'getServerTime' command.
     * It retrieves the current server time in milliseconds and sends it back to the sender using
     * the provided callback function in the message object.
     *
     * @param msg - The message object containing the command 'getServerTime' and the callback function.
     */
    getServerTimeMsg(msg) {
        this.ioUtil.logdebug('getServerDataMsg ');
