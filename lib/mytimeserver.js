...
function mytimeclass(adapter) {
    ...
    this.doCmd = function(idParts,state) {
        ...
        if ('+-\=#'.indexOf(value[0]) >= 0) {
            ...
            this.getCounddownData(idParts[0],function(data){
                ...
                this.setState('timer',data.timer,this.CountdownPath,idParts[0]);
            }.bind(this));
        ...
        if (value.toLowerCase() == 'start') {
            this.getCounddownData(idParts[0],function(data){
                ...
                if (data.action == 'pause') {
                    ...
                    this.setState('start',now,this.CountdownPath,idParts[0]);
                    this.setState('end',now+diff,this.CountdownPath,idParts[0]);                    
                    action = 'run';                    
                }
                if (data.action == 'stop') {
                    this.setState('start',now,this.CountdownPath,idParts[0]);
                    this.setState('end',now+data.timer,this.CountdownPath,idParts[0]);                    
                    action = 'run';
                }
                if (data.action == 'end') {
                    this.setState('start',now,this.CountdownPath,idParts[0]);
                ...
                this.setState('action',action,this.CountdownPath,idParts[0]);
            }.bind(this));
        }
        if (value.toLowerCase() == 'pause') {
            this.getCounddownData(idParts[0],function(data){
                ...
                if (data.action == 'stop') {
                    this.setState('start',0,this.CountdownPath,idParts[0]);
                    this.setState('end',0,this.CountdownPath,idParts[0]);                    
                    action = 'stop';
                }
                if (data.action == 'end') {
                    this.setState('start',0,this.CountdownPath,idParts[0]);
        ...
        if (value.toLowerCase() == 'setstop2timer') {
            this.getCounddownData(idParts[0],function(data){
                ...
                this.setState('config',JSON.stringify(data.config),this.CountdownPath,idParts[0]);
            }.bind(this));
        }
        ...
        this.setState('cmd',' ',this.CountdownPath,idParts[0]);
    }
    ...
    this.getCountdownInfo = function(msg,callback=null) {
        this.log.debug('getCountdownInfo ' + JSON.stringify(msg));
        this.adapter.getStates(this.adapter.namespace+'.*', (err, res) => {
            var data = {};
            for (let key in res) {
                var parts = key.split('.');
                if (parts[2]!='Countdown') continue;
                if (!data.hasOwnProperty(parts[3])) {
                    data[parts[3]]={};
                    data[parts[3]].name = parts[3];
                }                
                data[parts[3]][parts[4]]= res[key].val;
            }
